...
One concrete consequence of taking that asymmetry seriously: the gate is strict enough that even the spec's own canonical "clearly AI-generated" example (`requirements.md` → Milestone 4, the "Artificial intelligence represents a transformative paradigm shift..." text) doesn't reach `likely_ai` in this system. Logged result (`content_id 7ac25293…`): `llm_score = 0.92` (Groq is confident), `stylometric_score = 0.4331` (the structural signal isn't), combined `confidence = 0.5883` (59%) — `uncertain`, not `likely_ai`, because the asymmetric gate requires *both* signals to independently clear 0.55 and the stylometric signal doesn't. This is accepted as the cost of the design, not an oversight: the system would rather under-flag a piece of clearly-AI text than risk over-flagging a human's, and a single confident signal — even Groq at 0.92 — isn't allowed to outvote a skeptical second one.
